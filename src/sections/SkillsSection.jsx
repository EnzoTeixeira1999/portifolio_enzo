...
function SkillCard({ Icon, title, subtitle }) {
  return (
    <motion.div
      ...
      className="group rounded-xl border border-cyan-300/20 bg-white/[0.03] p-2 shadow-[0_0_14px_rgba(56,189,248,0.05)] transition hover:border-cyan-200/70 hover:bg-cyan-300/5 sm:rounded-2xl md:p-2"
    >
      <div className="flex items-center gap-2.5">
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-cyan-300/20 bg-cyan-300/10 text-cyan-200 md:h-8 md:w-8">
          <Icon size={15} />
        </div>
        ...
        <div className="min-w-0">
          ...
          <p className="mt-0.5 truncate text-[9px] text-zinc-500 md:text-[10px]">
            {subtitle}
          </p>
        </div>
      </div>
...
function SkillsSection({ language = "pt" }) {
  ...
  return (
    <section
      id="skills"
      className="bg-black px-4 pt-10 pb-14 text-white md:flex md:min-h-screen md:snap-start md:items-center md:px-6 md:py-8 lg:px-10"
    >
      <div className="mx-auto w-full max-w-7xl">
        <div className="mx-auto mb-6 max-w-3xl text-center">
          ...
          <h2 className="mt-3 text-3xl font-bold leading-tight sm:text-4xl md:text-4xl xl:text-5xl">
            {t.title}
          </h2>

          <p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-zinc-400 md:text-sm xl:text-base xl:leading-7">
            {t.text}
          </p>
        </div>
        ...
        <div className="grid gap-3 md:gap-3 lg:grid-cols-2">
          {sections.map((section, sectionIndex) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="rounded-2xl border border-white/10 bg-white/[0.025] p-3 shadow-[0_0_20px_rgba(0,0,0,0.16)] md:rounded-3xl md:p-3"
            >
              <h3 className="mb-2.5 text-sm font-semibold text-cyan-200 md:text-base">
                {section.title}
              </h3>
